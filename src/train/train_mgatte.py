...
from src.models.mgatte import build_mgatte
...
def train_mgatte(cfg: Dict, run_dir: Path | None = None) -> Path:
    set_seed(cfg["seed"])
    ...
    class_loss = tf.keras.losses.CategoricalCrossentropy(from_logits=cfg["train"].get("from_logits_loss", False))
    model.compile(
        optimizer=optimizer,
        loss=[class_loss, "mse"],
        loss_weights=[1.0, cfg["mgatte"]["regression_loss_weight"]],
    ...
    y_train_coords = np.stack([train_data["latitude"], train_data["longitude"]], axis=1)
    y_val_coords = np.stack([val_data["latitude"], val_data["longitude"]], axis=1)
    ...
    # Evaluate on test
    class_probs, coords_pred = model.predict(
        [X_test_input, test_data["words"], test_data["chars"]],
        batch_size=cfg["train"]["batch_size"],
    )
    ...
    if run_dir is None:
        run_dir = create_run_dir(cfg["paths"]["outputs_dir"], tag="mgatte")
    ...
    save_npz(model_dir / "preds.npz", y_true=y_true, y_pred=y_pred, y_prob=class_probs, coords_pred=coords_pred)
