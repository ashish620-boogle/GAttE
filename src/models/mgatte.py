...
def build_mgatte(
    ...
    if variant == "colab_v4":
        ...
        class_out = tf.keras.layers.Dense(num_classes, activation="softmax", name="class_output")(x)
        coord_out = tf.keras.layers.Dense(2, activation="linear", name="coord_output")(x)
    else:
        ...
        class_out = tf.keras.layers.Dense(num_classes, activation="softmax", name="class_output")(ff2)
        coord_out = tf.keras.layers.Dense(2, activation="linear", name="coord_output")(ff2)

    model = tf.keras.Model(inputs=inputs, outputs=[class_out, coord_out], name="M-GAttE")
    return model
